...
# Initialize session state for pdf_bytes
if 'pdf_bytes' not in st.session_state:
    st.session_state.pdf_bytes = None

# --- Minimal PDF Generation for Testing ---
def create_minimal_pdf():
    st.write("**Debugging: create_minimal_pdf() - START**") # Debugging line
    buffer = io.BytesIO()
    p = canvas.Canvas(buffer, pagesize=letter)
    p.drawString(100, 750, "Hello, World! This is a minimal PDF test.")
    p.save()
    pdf_bytes = buffer.getvalue()
...
# --- Streamlit UI ---
st.title("Minimal Download PDF Test")

uploaded_file = st.file_uploader("Upload DOCX/PDF (not used for PDF content in this test)", type=['docx', 'pdf'])

if uploaded_file:
    st.write("**Debugging: File Uploaded Detected**") # Debugging line
    st.write("**Debugging: Calling create_minimal_pdf()**") # Debugging line
    pdf_bytes = create_minimal_pdf()
    st.session_state.pdf_bytes = pdf_bytes
    st.write("**Debugging: pdf_bytes set in session state, pdf_bytes is None:", st.session_state.pdf_bytes is None) # Debugging line
else:
    st.write("**Debugging: No File Uploaded Yet**") # Debugging line
...
with st.sidebar:
    st.download_button(
        ...
        data=st.session_state.pdf_bytes if st.session_state.pdf_bytes else b'',
        file_name="minimal_document.pdf",
        mime="application/pdf",
        disabled=st.session_state.pdf_bytes is None
    )
    ...
    st.markdown("---")
    st.markdown("**Instructions:**")
    st.markdown("1. Upload a document (optional - content not used for PDF in this test)")
    st.markdown("2. Check if 'Download Minimal PDF' button becomes enabled.")
    st.markdown("3. Download and check the PDF.")
